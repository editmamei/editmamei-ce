...
- **Layer transforms & straightening** — move, scale, rotate, and fit-to-document on the active layer; background layers auto-promote instead of erroring, so you can straighten a tilted phone shot in one step
...
A template is a reproducible aesthetic recipe — capture a finished edit as a named bundle, then apply it later to new images. The **entire template surface is Pro**: authoring *and* use.
...
- `photoshop_template_list` — list saved templates
- `photoshop_template_apply` — apply a saved template to the current image; the AI re-derives each value for the new photo and self-judges against the template's exit criteria
- `photoshop_template_verify` — measure the current document against a template's machine-checkable style signature, with a corrective steer per miss
- `photoshop_template_recall` — re-surface one section of a template (exit criteria, tune dials, signature) as text, cheaply, late in a long session

Templates are the learning-loop layer — they turn one-shot edits into repeatable looks. Bundling the whole surface as one paid feature matches how it's used: authoring and applying are two halves of the same workflow.
